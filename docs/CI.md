...
O `gates-wired` carrega uma tabela de 24 gates, lida de `.local/phases/19-production-operations.md` e de `make verify`. Uma tabela, e não uma lista de alvos: um gate que sai da tabela some do CI em silêncio, e um gate que sai de `verify` mas continua na tabela é exatamente o que a regra pega.
...
- a análise estática, a de complexidade e a de código morto são as três metades baratas, todas só biblioteca padrão mais o analisador fixado, e todas no caminho rápido **e** no de release: `make lint` (P23-T02), `make audit-complexity` (P23-T03, que também aceita `-print-findings` para imprimir o baseline depois de um refactor — o portão nunca reescreve o arquivo por conta própria) e `make audit-deadcode` (P23-T04, que não tem baseline nenhum: a árvore não tem achado, e o achado futuro é uma recusa e não uma linha nova de débito);
