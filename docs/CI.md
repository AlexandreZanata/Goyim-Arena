...
O `gates-wired` carrega uma tabela de 23 gates, lida de `.local/phases/19-production-operations.md` e de `make verify`. Uma tabela, e não uma lista de alvos: um gate que sai da tabela some do CI em silêncio, e um gate que sai de `verify` mas continua na tabela é exatamente o que a regra pega.
